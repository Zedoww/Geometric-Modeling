...
void makeBuffers(myMesh *input_mesh)
{
	...
	{
		if (input_mesh->halfedges[i] == NULL || input_mesh->halfedges[i]->next == NULL)
			continue;
	...
	const bool have_tris = !verts.empty() && !norms.empty() && !norms_per_face.empty();
	const GLvoid *verts_ptr = have_tris ? static_cast<const GLvoid *>(&verts[0]) : nullptr;
	...
	// macOS Core Profile: have a VAO bound before any operation that touches VAO state.
	...
	glBufferData(GL_ARRAY_BUFFER, verts.size() * sizeof(GLfloat), verts_ptr, GL_STATIC_DRAW);

	glBindBuffer(GL_ARRAY_BUFFER, buffers[BUFFER_NORMALS_PERVERTEX]);
	glBufferData(GL_ARRAY_BUFFER, norms.size() * sizeof(GLfloat), norms_pv_ptr, GL_STATIC_DRAW);

	glBindBuffer(GL_ARRAY_BUFFER, buffers[BUFFER_NORMALS_PERFACE]);
	glBufferData(GL_ARRAY_BUFFER, norms_per_face.size() * sizeof(GLfloat), norms_pf_ptr, GL_STATIC_DRAW);
...
void draw_text(GLfloat x, GLfloat y, GLfloat z, string text, vector<GLfloat> color)
{
	...
	for (unsigned char ch : text)
		glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, static_cast<int>(ch));
...
void keyboard(unsigned char key, int x, int y)
{
	...
	case 'h':
		...
		break;
#if defined(__APPLE__)
	/* Remplace la molette (FreeGLUT) absente du GLUT Apple */
	case '+':
	case '=':
		camera_eye += camera_forward * 0.02;
		break;
...
{
	glutInit(&argc, argv);
	/* Do not request GL 3.2 Core on macOS: this app still uses fixed-function (glBegin, selection buffer, etc.). */
	...
	if (glew_err != GLEW_OK)
	{
		cerr << "glewInit failed: " << glewGetErrorString(glew_err) << "\n";
		exit(1);
	}
	// GLEW emits GL_INVALID_ENUM on core contexts; ignore that single error.
	glGetError();
	...
	glutAddMenuEntry("Triangulate (ear clipping)", MENU_TRIANGULATE);
